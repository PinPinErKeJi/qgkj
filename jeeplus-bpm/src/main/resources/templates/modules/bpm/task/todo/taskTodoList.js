                       ...
                       formatter: function operateFormatter(value, row, index) {
						   var arra = new Array();
                       	  if(row['task.assignee'] == undefined || row['task.assignee'] == ""){
                       	  	...
                       	  	arra.push('<a href="${ctx}/act/task/form?taskId='+row['task.id']+'&taskName='+encodeURI(row['task.name'])+'&taskDefKey='+row['task.taskDefinitionKey']+'&procInsId='+row['task.processInstanceId']+'&procDefId='+row['task.processDefinitionId']+'&status='+row.status+'" data-pjax>任务办理</a>');

						  };
                           <% if(shiro.hasPermission("act:process:edit")){ %>
                               if(row['task.executionId'] == undefined || row['task.executionId'] == ""){
                                   arra.push( '<div class="jp-divider jp-divider-vertical"></div>');
                       	  			arra.push( '<a href="${ctx}/act/task/deleteTask?taskId='+row['task.id']+'&reason=" onclick="return jp.prompt(\'删除原因\',this.href);">删除任务</a>')
                               };

                           <% } %>
                           arra.push( '<div class="jp-divider jp-divider-vertical"></div>');
                           arra.push('<a  href="javascript:jp.openViewDialog(\'进度\',\'${ctx}/act/task/trace/photo/'+row['task.processDefinitionId']+'/'+row['task.executionId']+'\',\'1000px\', \'600px\')">进度</a>');
