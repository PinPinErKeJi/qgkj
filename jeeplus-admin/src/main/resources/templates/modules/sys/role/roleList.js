			...
			$('#table').bootstrapTable({
					  //请求方法
		                ...
		                columns: [{
					        ...
					        title: '状态',
					        formatter:function(value, row , index){
					        	return value=="0" ? '<font  class="jp-tag jp-tag-has-color" style="background-color: rgb(255, 85, 0);">禁用</font>':'<font class="jp-tag jp-tag-has-color" style="background-color: rgb(135, 208, 104);">正常</font>';
					        }
					    }, {
	                        field: 'operate',
	                        title: '操作',
	                        ...
	                        formatter:  function operateFormatter(value, row, index) {
	            		        return [
									...
									<% if(shiro.hasPermission("sys:role:assign")){ %>
	        							'<a href="#" class="auth">角色授权</a>',
                                        '<div class="jp-divider jp-divider-vertical"></div>',
									<% } %>
									<% if(shiro.hasPermission("sys:role:assign")){ %>
	        							'<a href="#" class="assign">分配用户</a>'
									<% } %>
	            		        ].join('');
	            		    }
	                    }]
